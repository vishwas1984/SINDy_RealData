...
class EmpGPMJOPred:
    ...
    def pred(self, test_on_choose_dir=True, test_ids=None):
        ...
        for width in self.widths:
            ...
            if test_ids is None:
                self.test_ids[width] = s2s_data[self.choose_dir_name]['ensemble_mean_rmm1.nc']['S'][-self.n_pred : ] - width
            else:
    ...
    def add_s2s(self, dir_names, hdate_id=19):
    ...
    def add_nn(self, model_clses=[FFNNModel], width=300, lead_time=60, 
            hidden_dim=64, num_epochs=10, lr=0.01, verbose=True):
        npzfile = self.npzfile
        n_pred = self.n_pred
        emp_model = self.emp_model
    ...
    def plot_metrics(self, nrows=2, ncols=3):
        ...
        fig.legend(lines, labels, fontsize=50, loc='lower center', bbox_to_anchor=(0.53, -0.06),
                    ncol=2*num_legends, fancybox=True, shadow=True)

        parent_dir_path = os.path.dirname(os.path.dirname(os.path.dirname(os.path.abspath(__file__)))) # ../SINDy_RealData
        file_path = os.path.join(parent_dir_path, "figs", "metrics", f"metrics_compare.png")
    ...
    def plot_ts(self, nrows=1, ncols=2, hdate_ids=[9, 12, 15, 19], pred_id=0):
        ...
        n_pred = self.n_pred
        max_lead_time = max( self.lead_time,  s2s_data[dir_name]['ensemble_mean_rmm1.nc']['RMM1'].shape[-1])
